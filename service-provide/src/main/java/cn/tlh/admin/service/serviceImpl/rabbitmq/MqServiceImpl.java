...
public class MqServiceImpl implements MqService, RabbitTemplate.ConfirmCallback, RabbitTemplate.ReturnCallback {
    ...
    public void confirm(CorrelationData correlationData, boolean ack, String cause) {
        ...
        if (ack) {
            ...
            brokerMessageLogDao.changeBrokerMessageLogStatus(messageId, RabbitMqConstants.ORDER_SEND_SUCCESS, new Date());
        } else {
...
//            message.getMessageProperties().setDelay(times);
        ...
        rabbitTemplate.convertAndSend(
                ...
                dlxMessage,
                processor);
        log.info("延时队列发送：队列名：{}，消息：{}，延时时间（毫秒）：{}", queueName, msg, times);
    }
